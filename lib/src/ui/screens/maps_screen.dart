...
class _MapScreenState extends State<MapScreen> {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        body: ConstrainedBox(
          ...
          child: Stack(
            children: [
              Selector2<MapsProvider, AbsenProvider,
                  Tuple2<Position, DestinasiModel>>(
                ...
                builder: (_, value, __) {
                  return GoogleMap(
                    ...
                    initialCameraPosition: CameraPosition(
                      target:
                          LatLng(value.item1.latitude, value.item1.longitude),
                      zoom: 14.4746,
                    ),
              ...
              Positioned(
                ...
                left: 10,
                right: 50,
                child: Selector2<MapsProvider, AbsenProvider,
                    Tuple2<Position, DestinasiModel>>(
                  selector: (_, provider1, provider2) => Tuple2(
                      provider1.currentPosition, provider2.destinasiModel),
                  builder: (_, value, __) {
        ...
        floatingActionButton: Padding(
          padding:
              EdgeInsets.only(top: sizes.statusBarHeight(context) * 4 + 10),
          child: InkWell(
    ...
    _positionStream = positionStream.listen((Position position) {
      ...
      print(
        ...
        onError: (dynamic error) =>
            print('Error Handling Listen Stream ${error.toString()}'));
  }
  ...
  Future<void> _validateAbsen({
    ...
    final isInsideRadius =
        commonF.isInsideRadiusCircle(distanceTwoLocation, radius);
      ...
      if (isInsideRadius) {
        try {
          ...
          await globalF.showToast(
              message: result, isSuccess: true, isLongDuration: true);
          globalProvider.setLoading(false);
          await Navigator.of(context)
              .pushReplacementNamed(WelcomeScreen.routeNamed);
        } catch (e) {
          await globalF.showToast(
              message: e.toString(), isError: true, isLongDuration: true);
          globalProvider.setLoading(false);
        }
      } else {
        await globalF.showToast(
            message: 'Anda Diluar Jangkauan Absensi', isError: true);
      }
    } else {
      await globalF.showToast(
